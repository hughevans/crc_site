# 18/2/2008
# I've added to the Regex to allow apostrophes in the address part of the email
# I've also added the :allow_blank option
    ...
    module ClassMethods
      ...
      def validates_email_format_of(*attr_names)
        configuration = { :message => ' does not appear to be a valid e-mail address', 
                          :on => :save, 
                          :with => /^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++)|([A-Za-z0-9]+\'+))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6}$/i,
                          :allow_blank => false }
